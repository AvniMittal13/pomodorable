...
import { doc, getDoc, updateDoc, serverTimestamp, Timestamp, setDoc, DocumentData, onSnapshot } from 'firebase/firestore';
...
import { ArrowLeft, Save, Edit2, CheckSquare, Coffee, Briefcase, CheckCircle, Loader2 } from 'lucide-react';
...
interface PomodoroSessionData {
  ...
  userId: string;
  todos?: any[]; // Will be Task[] from TodoList component
  mood?: string | null;
  dailyGoals?: string;
  // moodLastSetTime?: Timestamp; // For MoodTracker display
  // dailyGoalsLastSaved?: Timestamp; // For StickyNote display
}
...
export default function ActiveSessionPage() {
  ...
  // Fetch session data with real-time updates
  useEffect(() => {
    if (!user || !sessionId) {
      if (!sessionId && !user) setLoading(false);
      return;
    }
    ...
    return () => unsubscribeSession(); // Cleanup snapshot listener
  ...
  const handleSaveName = async () => {
    if (!sessionData || !sessionNameInput.trim() || sessionNameInput.trim() === sessionData.sessionName) {
      ...
      if (sessionData) setSessionNameInput(sessionData.sessionName); // Reset to original if no change
      return;
    }
    const sessionDocRef = doc(db, 'pomodoroSessions', sessionId);
    try {
  ...
  const handleSessionComplete = useCallback(async () => {
    ...
    // Toast is less intrusive as other components save continuously
    // toast({ title: "Session Over!", description: "Finalizing your session data...", duration: 3000 });
  ...
  if (!sessionData || !user) {
    // Fallback, should be handled by redirects in useEffect
    return (
        <div className="min-h-screen flex items-center justify-center bg-background">
            <p>Session not found, access denied, or user not authenticated.</p>
        </div>
    );
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 p-4 sm:p-6 lg:p-8 selection:bg-primary/20">
      <header className="container mx-auto max-w-6xl mb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          ...
          <div className="text-sm text-muted-foreground mt-1 sm:mt-0 text-right">
            <p>Status: <span className={`font-semibold ${isSessionCompleted ? 'text-green-600' : 'text-blue-600'}`}>{sessionData.status.charAt(0).toUpperCase() + sessionData.status.slice(1)}</span></p>
      ...
      <div className="container mx-auto max-w-6xl grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        <div className="md:col-span-2 xl:col-span-1">
          <PomodoroTimer 
            key={`${sessionId}-${sessionData.status}`} // Re-key to force re-render if status changes to ensure timer state correct
